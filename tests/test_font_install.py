...
class TestFontInstall(unittest.TestCase):
    ...
    def test_find_default_font_finds_windows_system_font(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            windows_root = Path(tmpdir)
            windows_font = windows_root / "Fonts" / "msyh.ttc"
            windows_font.parent.mkdir()
            windows_font.write_bytes(b"windows-font")

            with (
                patch.dict(os.environ, {"WINDIR": str(windows_root)}, clear=False),
                patch(
                    "data.plugins.astrbot_plugin_link_resolver.core.common.card_renderer.utils.get_user_font_paths",
                    return_value=ManagedFontPaths(primary=None, emoji=None),
                ),
                ...
                patch(
                    "data.plugins.astrbot_plugin_link_resolver.core.common.card_renderer.utils._font_path_loadable",
                    side_effect=lambda candidate, size: candidate == windows_font,
                ),
            ):
                self.assertEqual(find_default_font(), windows_font)
