...
* **原版**：仅支持原图下载/转码发送。
* **魔改版**：针对低分辨率（低于设定阈值，如 1080px）或模糊度不达标的图片，自动触发本地 Upscayl 双重 Pass 渲染与 TAA 抗锯齿，大幅提升画质。
...
* **原版**：后台无详细百分比进度，无查询指令。
* **魔改版**：
* 提供 `/解析进度`、`/生图进度`、`/小红书进度` 命令，实时查看当前处理的图片序号、AI 升图阶段、百分比及已耗时。
...
| `解析进度` / `生图进度` / `小红书进度` | 查询当前正在后台执行的 AI 升图/AVIF 压制/多图处理实时进度 | `/解析进度` |
