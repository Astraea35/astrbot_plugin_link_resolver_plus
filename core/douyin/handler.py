                ...
                for i, img_path in enumerate(image_paths):
                    await prepare_metadata(
                        img_path,
                        {
                            ...
                            "paired_video_url": dynamic_urls[i] if i < len(dynamic_urls) else None,
                            "live_photo": bool(i < len(dynamic_urls)),
                        },
                    )

            # 3. 统一图片后处理：按平台配置决定是否升图和生成预览。
            if image_paths:
                if getattr(self, "douyin_enable_ai_upscale", True):
                ...
                image_paths, image_processing_metadata, avif_files = (
                    await BaseUtilsMixin._process_image_collection(
                        ...
                        request_id,
                        auto_upscale=(
                            "douyin_enable_ai_upscale",
                            "douyin_low_quality_threshold",
                            "douyin_upscayl_model_name",
                        ),
