...
class TwitterMixin:
    ...
    async def _process_twitter(
        ...
        # 3. 统一图片后处理：按平台配置决定是否升图和生成预览。
        if image_paths:
            if getattr(self, "twitter_enable_ai_upscale", True):
            ...
            image_paths, image_processing_metadata, avif_files = (
                await BaseUtilsMixin._process_image_collection(
                    ...
                    request_id,
                    auto_upscale=(
                        "twitter_enable_ai_upscale",
                        "low_quality_threshold",
                        "twitter_upscayl_model_name",
                    ),
            ...
            if not getattr(self, "enable_global_ffmpeg_compress", True):
                logger.info("ℹ️ X 全局图片压缩未启用，跳过转码文件生成与发送")
        ...
        try:
            ...
            if avif_files_to_send:
                logger.info("📁 X 准备独立发送 %d 个图片转码文件", len(avif_files_to_send))
            for avif_file in avif_files_to_send:
                try:
                    if not await self._send_file_via_api(event, avif_file):
                        logger.warning("⚠️ X 独立发送图片转码文件失败: %s", avif_file.name)
                except Exception as exc:
                    logger.warning("⚠️ X 独立发送图片转码文件失败 (%s): %s", avif_file.name, str(exc))
