...
class MediaConcurrencyController:
    """AI 升图与 FFmpeg 任务的全局并发调度器。

    - ai_upscale_slot / ffmpeg_slot：分别限制两类重任务的并行数；
    - whole_job：整条处理流程（如图片工具）占用全局互斥或两类槽位；
    - 配置在获取槽位时实时读取，修改后立即生效，无需重建。
    """

    def __init__(self, plugin_instance) -> None:
        self.plugin = plugin_instance
        self._condition = asyncio.Condition()
        self._active = {"ai_upscale": 0, "ffmpeg": 0, "job": 0}

    @staticmethod
    def _read_limit(plugin: object, group: str) -> int:
        if group == "ai_upscale":
            return max(1, int(getattr(plugin, "ai_upscale_max_concurrent", 1)))
        return max(1, int(getattr(plugin, "ffmpeg_max_concurrent", 1)))

    @staticmethod
    def _allow_together(plugin: object) -> bool:
        return bool(getattr(plugin, "allow_ai_upscale_ffmpeg_concurrent", False))

    async def _acquire(self, group: str) -> None:
        other = "ffmpeg" if group == "ai_upscale" else "ai_upscale"
        async with self._condition:
            while True:
                if self._active["job"] > 0:
                    await self._condition.wait()
                    continue
                if self._active[group] >= self._read_limit(self.plugin, group):
                    await self._condition.wait()
                    continue
                if not self._allow_together(self.plugin) and self._active[other] > 0:
                    await self._condition.wait()
                    continue
                self._active[group] += 1
                return

    async def _release(self, group: str) -> None:
        async with self._condition:
            self._active[group] = max(0, self._active[group] - 1)
            self._condition.notify_all()

    @asynccontextmanager
    async def ai_upscale_slot(self):
        await self._acquire("ai_upscale")
        try:
            yield
        finally:
            await self._release("ai_upscale")

    @asynccontextmanager
    async def ffmpeg_slot(self):
        await self._acquire("ffmpeg")
        try:
            yield
        finally:
            await self._release("ffmpeg")
        ...
        async with self._condition:
            while True:
                if self._active["job"] > 0:
                    ...
                    continue
                if self._allow_together(self.plugin):
                    if self._active["ai_upscale"] >= self._read_limit(self.plugin, "ai_upscale"):
                        await self._condition.wait()
                        continue
                    if self._active["ffmpeg"] >= self._read_limit(self.plugin, "ffmpeg"):
                        await self._condition.wait()
                        continue
                elif self._active["ai_upscale"] > 0 or self._active["ffmpeg"] > 0:
                    await self._condition.wait()
                    continue
                self._active["job"] = 1
                break
...
class BaseUtilsMixin:
    ...
    def _is_session_enabled(self, event: AstrMessageEvent) -> bool:
        if not getattr(self, 'session_control_enabled', True):
            return True
        disabled = getattr(self, '_disabled_parse_sessions', set())
        return self._get_session_key(event) not in disabled

    def _set_session_enabled(self, event: AstrMessageEvent, enabled: bool) -> bool:
        if not getattr(self, 'session_control_enabled', True):
            return False
        if not hasattr(self, '_disabled_parse_sessions'):
            self._disabled_parse_sessions: set[str] = set()
        key = self._get_session_key(event)
        if enabled:
            self._disabled_parse_sessions.discard(key)
        else:
            self._disabled_parse_sessions.add(key)
        return True

    def _event_text_for_debounce(self, event: AstrMessageEvent) -> str:
        payloads: list[str] = []
        for source in (event, getattr(event, 'message_obj', None)):
            ...
            for attr in ('message_str', 'message', 'raw_message'):
                ...
                if isinstance(value, str):
                    ...
                    try:
                        payloads.append(json.dumps(value, ensure_ascii=False, sort_keys=True))
                    except (TypeError, ValueError):
                        payloads.append(str(value))
        return '\n'.join(payloads).strip()

    def _is_event_debounced(self, event: AstrMessageEvent, platform: str) -> bool:
        debounce_seconds = max(0, int(getattr(self, 'debounce_seconds', 0)))
        if debounce_seconds == 0:
            return False

        text = self._event_text_for_debounce(event)
        if not text:
            return False
        now = time.monotonic()
        if not hasattr(self, '_link_debounce_cache'):
            self._link_debounce_cache: dict[str, float] = {}
        cache = self._link_debounce_cache
        expired = [key for key, stamp in cache.items() if now - stamp >= debounce_seconds]
        for key in expired:
            cache.pop(key, None)

        digest = hashlib.sha1(text.encode('utf-8')).hexdigest()
        key = f'{self._get_session_key(event)}:{platform}:{digest}'
        if key in cache:
            logger.info('⏭️ %s 链接命中 %s 秒防抖，跳过重复解析', platform, debounce_seconds)
            return True
        cache[key] = now
        return False

    def _can_process_event(self, event: AstrMessageEvent, platform: str) -> bool:
        if not self._is_message_allowed(event):
            return False
        if not self._is_session_enabled(event):
            logger.debug('⏭️ 当前会话已关闭链接解析，跳过 %s', platform)
            return False
        return not self._is_event_debounced(event, platform)
    ...
    @property
    def media_concurrency(self) -> MediaConcurrencyController:
        if not hasattr(self, "_media_concurrency_obj"):
            self._media_concurrency_obj = MediaConcurrencyController(self)
        return self._media_concurrency_obj

    @property
    def ai_upscale_slot(self):
        return self.media_concurrency.ai_upscale_slot()

    @property
    def ffmpeg_slot(self):
        return self.media_concurrency.ffmpeg_slot()

    @property
    def media_whole_job(self):
        return self.media_concurrency.whole_job()
