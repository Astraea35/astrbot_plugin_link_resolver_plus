...
GENERAL_SETTING_SECTIONS = {
    "upscayl_bin_path": "ai_upscale",
    "upscayl_models_path": "ai_upscale",
    "upscayl_double_pass": "ai_upscale",
    ...
    "reaction_emoji_strategy": "message_behavior",
    "auto_install_fonts": "font_rendering",
    "custom_font_path": "font_rendering",
    "custom_emoji_font_path": "font_rendering",
    "max_video_size_mb": "media_delivery",
    "napcat_media_share_path": "media_delivery",
    "napcat_media_container_path": "media_delivery",
    "merge_send_as_sender": "media_delivery",
...
class ConfigMixin:
    ...
    def _get_general_config_value(self, key: str, default):
        general_settings = self._get_config_value("general_settings", {})
        if not isinstance(general_settings, dict):
            return default

        section = GENERAL_SETTING_SECTIONS.get(key)
        if section:
            section_settings = general_settings.get(section)
            if isinstance(section_settings, dict):
                value = section_settings.get(key)
                if value is not None:
                    return value

        legacy_value = general_settings.get(key)
    ...
    def _configure_managed_fonts(self) -> None:
        ...
        custom_primary_font = str(
            self._get_general_config_value("custom_font_path", "")
        ).strip()
        custom_emoji_font = str(
            self._get_general_config_value("custom_emoji_font_path", "")
        ).strip()
        ...
        self.font_auto_install_enabled = bool(
            self._get_general_config_value("auto_install_fonts", False)
        )
    ...
    def _refresh_config(self) -> None:
        ...
        if self._get_config_value('enable_platforms', None) is None:
            enable_platforms.extend(EXTENDED_PLATFORM_LABELS)
        self.enabled_platforms = {str(item).strip() for item in enable_platforms}
        ...
        self.douyin_enable_ai_upscale = bool(
            self._get_config_value(
                "douyin_settings.enable_ai_upscale",
                self._get_config_value("general_settings.enable_ai_upscale", True),
            )
        )
        ...
        self.weibo_enable_ai_upscale = bool(
            self._get_config_value(
                "weibo_settings.enable_ai_upscale",
                self._get_config_value("general_settings.enable_ai_upscale", True),
            )
        )
        ...
        self.twitter_enable_ai_upscale = bool(
            self._get_config_value(
                "twitter_settings.enable_ai_upscale",
                self._get_config_value("general_settings.enable_ai_upscale", True),
            )
        )
        ...
        self.upscayl_double_pass = bool(
            self._get_general_config_value("upscayl_double_pass",
                self._get_config_value("xhs_settings.upscayl_double_pass", True),
            )
        )
        ...
        user_bin = str(
            self._get_general_config_value("upscayl_bin_path", DEFAULT_UPSCAYL_BIN_PATH
            )
        ).strip().strip('"').strip("'")
        user_models = str(
        ...
        self.low_quality_threshold = max(
            100,
            int(
                self._get_general_config_value("low_quality_threshold",
                    self._get_config_value("xhs_settings.low_quality_threshold", 2160),
                )
            ),
        )
        self.upscayl_enable_taa = bool(
            self._get_general_config_value("upscayl_enable_taa",
                self._get_config_value("xhs_settings.upscayl_enable_taa", True),
            )
        )
        self.upscayl_scale = min(
            4,
            max(
                1,
                int(
                    self._get_general_config_value("upscayl_scale",
                        self._get_config_value("xhs_settings.upscayl_scale", 2),
                    )
                ),
            ),
        )
        ...
        self.ffmpeg_bin_path = str(self._get_general_config_value("ffmpeg_bin_path", "ffmpeg")).strip()
        # 全局并发设置：AI 升图与 FFmpeg 是否可同时运行、各自并行任务数
        self.allow_ai_upscale_ffmpeg_concurrent = bool(
            self._get_general_config_value("allow_ai_upscale_ffmpeg_concurrent", False)
        )
        self.ai_upscale_max_concurrent = max(
            1, int(self._get_general_config_value("ai_upscale_max_concurrent", 1))
        )
        ...
        self.retry_count = max(
            0, int(self._get_general_config_value("retry_count", 3))
        )
        self.reaction_emoji_enabled = bool(
            self._get_general_config_value("reaction_emoji_enabled", True)
        )
        _raw_list = self._get_general_config_value("reaction_emoji_list", [127827]
        )
        ...
        _strategy = str(
            self._get_general_config_value("reaction_emoji_strategy", "随机")
        ).strip()
        ...
        self.max_video_size_mb = int(
            self._get_general_config_value("max_video_size_mb", 200)
        )
        self.napcat_media_share_path = str(
            self._get_general_config_value("napcat_media_share_path", "")
        ).strip().strip('"').strip("'")
        self.napcat_media_container_path = str(
            self._get_general_config_value("napcat_media_container_path", "")
        ).strip().strip('"').strip("'")
        self.merge_send_as_sender = bool(
            self._get_general_config_value("merge_send_as_sender", False)
        )
        _mode = str(
            self._get_general_config_value("error_notify_mode", "静默")
        ).strip()
        self.error_notify_mode = _mode if _mode in ("静默", "脱敏", "报错") else "静默"

        self.debounce_seconds = max(
            0, min(3600, int(self._get_general_config_value("debounce_seconds", 30)))
        )
        self.session_control_enabled = bool(
            self._get_config_value('session_control.enabled', True)
        )
        self.extended_proxy_url = str(
            self._get_config_value('extended_platforms_settings.proxy_url', '')
        ).strip()
        self.extended_cookies_file = str(
            self._get_config_value('extended_platforms_settings.cookies_file', '')
        ).strip().strip('"').strip("'")
        self.extended_download_timeout = max(
            30,
            min(900, int(self._get_config_value('extended_platforms_settings.download_timeout', 120))),
        )
        self.extended_max_media = max(
            1, min(99, int(self._get_config_value('extended_platforms_settings.max_media', 10)))
        )
        self.extended_gallery_dl_fallback = bool(
            self._get_config_value('extended_platforms_settings.gallery_dl_fallback', True)
        )
        self.extended_send_summary = bool(
            self._get_config_value('extended_platforms_settings.send_summary', True)
        )
