...
class CommandsMixin:
    ...
    @filter.command('开启解析')
    async def cmd_enable_session_parser(self, event: AstrMessageEvent):
        if not self._set_session_enabled(event, True):
            yield event.plain_result('会话级解析开关未启用，请在插件设置中开启。')
            return
        yield event.plain_result('✅ 当前会话已开启链接解析。')

    @filter.command('关闭解析')
    async def cmd_disable_session_parser(self, event: AstrMessageEvent):
        if not self._set_session_enabled(event, False):
            yield event.plain_result('会话级解析开关未启用，请在插件设置中开启。')
            return
        yield event.plain_result('⏸️ 当前会话已关闭链接解析；发送“开启解析”可恢复。')

    @filter.command('解析状态')
    async def cmd_session_parser_status(self, event: AstrMessageEvent):
        if not getattr(self, 'session_control_enabled', True):
            yield event.plain_result('会话级解析开关未启用，当前解析状态由全局平台设置决定。')
            return
        status = '已开启' if self._is_session_enabled(event) else '已关闭'
        yield event.plain_result(f'当前会话的链接解析：{status}。')
