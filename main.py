...
from .core.extended_platforms import EXTENDED_MESSAGE_PATTERN
from .core.extended_platforms.handler import ExtendedPlatformsMixin
...
class LinkResolverPlugin(
    ...
    async def handle_bili_video(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, 'B站'):
            return
    ...
    async def handle_douyin(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, '抖音'):
            return
    ...
    async def handle_xhs(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, '小红书'):
            return
    ...
    async def handle_weibo(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, '微博'):
            return
    ...
    async def handle_twitter(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, 'X'):
            return
        self._register_parse_task("twitter", event)
        await TwitterMixin.handle_twitter(self, event)

    @filter.regex(EXTENDED_MESSAGE_PATTERN, priority=9)
    async def handle_extended_platforms(self, event: AstrMessageEvent):
        self._refresh_config()
        if self._has_json_component(event) or not self._can_process_event(event, '扩展平台'):
            return
        await self._process_extended_event(event)
    ...
    async def handle_json_card(self, event: AstrMessageEvent):
        ...
        if not has_json_component or await self._is_bot_muted(event) or not links:
            return
        if not self._can_process_event(event, '卡片'):
            return
