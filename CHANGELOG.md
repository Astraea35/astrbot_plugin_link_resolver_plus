# Changelog

## v1.10.4

- 新增小红书视频与实况图短视频（Live Photo MP4）智能自适应去水印功能：
  - 采用本地 FFmpeg 智能自适应 `delogo` 滤镜，根据横屏/竖屏视频分辨率精准动态计算水印边界，平滑消除右下角的小红书官方水印。
  - 完全离线本地运算（耗时通常小于 2 秒），无需第三方解析接口，稳定可靠零依赖。
  - 自动复用全局 FFmpeg 并发调度槽位，避免转码突发抢占系统资源。
  - 新增 `xhs_settings.remove_watermark` 开关配置（默认开启），支持在管理面板随时开启或关闭。

## v1.10.3

- 深度适配 NapCat 异机/容器部署（如 NapCat 在 NAS、Docker，AstrBot 在 Windows 本机）：小红书实况短视频在未配置容器挂载共享路径时自动转 Base64 内存流发送，穿透异机文件系统限制，彻底解决 `识别URL失败, uri= C:\...` 报错。
- 新增 `enable_live_photo_video` 配置项（默认开启）：允许用户按需关闭实况短视频下载与发送，仅保留高清纯图。
- 强化发送分段异常隔离：在卡片事件分段发送（`event.send`）循环中增加异常捕获，即使单个短视频发送受限，也不会影响图文合并转发和主交互流程。


- 优化小红书图文与 Live Photo（实况照片）发送机制：将实况短视频与图文合并转发剥离，合并转发节点仅打包文字摘要与图片，避免因 QQ 协议端处理转发视频失败导致图文整体中断。
- 实况短视频在合并转发送达后以独立消息逐条发送，并加入异常隔离保护。
- 修复 Windows 下 NapCat 协议端解析本地文件路径时开头的斜杠解析 Bug（`open '/C:/...'` ENOENT 报错）。

## v1.10.1
- B站、抖音和小红书的媒体文件在发送失败、任务取消或结果链提前关闭时也会清理；卡片保存取消后不再残留文件，B站卡片文件名按请求隔离。
- 补充 B站无协议短链、抖音 `modal_id`、微博视频、小红书用户主页笔记和 X 长文图片等链接与媒体识别。

## v1.10.0

- 新增双机部署支持：可将高负载的 AI 升图（Upscayl / AnimeJaNai / SPAN）与 FFmpeg 转码（AVIF / JXL / 预览生成）通过 HTTP 原生二进制文件流卸载到 PC 独立算力节点处理。
- 100% 保持单机模式兼容：关闭开关时完全回退为本地独立运行，对既有单机环境零侵入。
- 新增 `remote_worker` 算力服务工程与 Windows 一键启动脚本（FastAPI + Uvicorn），支持可选 Token 密钥鉴权与自动工具/模型路径探测。
- 独家深度优化保护 Minecraft 等宿主服务器：PC 算力节点默认开启严格单任务串行排队（`max_concurrency: 1`），自动硬锁定至 Windows E 核（小核掩码 `0xFF000`）并以 `IDLE` 空闲优先级运行，彻底隔离大核（P核），保障游戏服务器 TPS 绝对稳定。
- 新增失败容错策略开关：支持“优雅容错 (降级发送原图)”与“严格报错 (中断并提示异常)”，可在管理面板按需自选。
- 新增双机通信与降级容错单元测试，并通过全部 56 项媒体与架构回归测试。

## v1.9.1

- 自动动漫升图的 AnimeJaNai Balanced 改为连续原生 2x，直到最长边首次达到或超过 `3840px`；最终输出不再缩回 3840px。
- 新增 `animejanai_auto_min_long_edge` 与 `animejanai_auto_max_passes`，默认最低长边 `3840px`、最多 `6` 轮，极小图片达到安全上限时保留现有原生输出并提示。
- 内置 AnimeJaNai DirectML 运行器支持重叠分块推理，自动使用 `768px` 分块和 `64px` 重叠；多轮升图可在 8GB 显存显卡上稳定执行。
- AnimeJaNai 自动缓存键加入轮数和最低长边，旧的单轮、已回缩缓存不会被复用。

## v1.9.0

- 自动图片分类升级为规则 V2 + LAION CLIP ViT-B/32 低置信度复核，输出 `anime`、`photo`、`text_ui`、`uncertain` 四类结构化结果。
- 文字、聊天、网页、文档和应用界面截图跳过 AI 升图；动漫自动使用 AnimeJaNai Balanced，照片沿用 LiveAction/Nomos8k 动态路由，不确定图片使用 Remacri。
- CLIP 视觉 FP16 ONNX 不进入插件仓库，首次启动后台下载到实例级 `models/classifier`；源码固定资源与清单 SHA256，目录锁和唯一临时文件避免重载竞争，校验成功后原子替换。
- DirectML Session 使用顺序执行和单线程 `run()`；多图先整组分类，首次升图前立即释放，空闲 30 秒自动卸载，插件停止时最多等待 10 秒清理资源。
- 新增规则阈值、UI 检测、CLIP 置信度、资源损坏、并发串行、空闲卸载、请求续期、升图前释放与图片工具锁顺序测试。

## v1.8.10

- 自动动漫升图统一优先 AnimeJaNai V3.1 Balanced；HFA2k SPAN 保留为手动动漫自然 2x 选项。

## v1.8.9

- SPAN 和 Upscayl 的运行器、模型也改为迁移到 AstrBot 实例目录的 `ai_upscale`，避免插件或实例更新后自动升图回退。

## v1.8.8

- AnimeJaNai 权重改存放在 AstrBot 实例根目录的 `models/animejanai`；自动迁移 v1.8.7 的插件数据目录和旧版插件资源，实例版本升级也不会覆盖模型。

## v1.8.7

- AnimeJaNai 权重默认保存到插件数据目录；首次启动自动迁移旧版插件资源，后续 GitHub 更新不再覆盖模型文件。

## v1.8.6

- 修正 AnimeJaNai 首次更新后权重未解压导致的失败提示；模型缺失时会明确写出所需文件和目录，子进程失败也会保留关键错误详情。

## v1.8.5

- AnimeJaNai Balanced 新增内置 DirectML 运行器和自动依赖，可直接使用本地 ONNX 权重；自动动漫路由优先 Balanced，AVV3 保留手动选择。
- 移除未打包运行环境和权重的 HATGAN 选项；照片仍使用内置 Nomos8k SPAN 4x 高质量模型。

## v1.8.4

- AI 升图升级为 Upscayl/Real-ESRGAN 与 SPAN 双后端：自动识别动漫和照片，并按原图长边动态选择 2x/4x 模型，默认将输出长边限制为 3840px。
- 自动动漫模型新增 HFA2k SPAN 2x 与 AnimeVideoV3 4x；自动照片模型新增 LiveActionV1 SPAN 2x 与 Nomos8k SPAN 4x。
- 新增 AnimeJaNai V3.1 Balanced/Sharp 手动高质量档，随资源包提供非商业许可的 ONNX 权重；执行时需自行配置 ONNX/TensorRT/DirectML 运行器。
- 新增 Real HAT GAN x4/Sharper 手动最高质量档的外部运行器接口；HAT 权重和运行器不随包提供，未配置时自动回退 Remacri。
- `ultrasharp-4x` 不再参与自动路由，仍保留手动选择；原有 `digital-art-4x`、`remacri-4x`、`upscayl-lite-4x` 等模型继续作为兼容选项和故障回退。
- 升图缓存键加入后端、模型、倍率和 TAA 状态，避免不同模型误用旧缓存；独立 `/升图 自动` 同样启用动态倍率和 3840px 输出保护。
- 更新第三方授权说明，补充 AnimeJaNai、SPAN NCNN、LiveActionV1 与 Real-ESRGAN/AnimeVideoV3 的来源和许可证信息。

## v1.8.3

- v1.8.3 自动更新

## v1.8.2

- v1.8.2 自动更新

## v1.8.1

- 新增jxl

## v1.8.0

- 抖音新增免登录游客会话解析回退，接口返回空数据时自动刷新会话重试
- 支持下载抖音动图和小红书 Live Photo 视频

## Unreleased

- 抖音：同步上游签名游客详情接口。常规接口空数据、网络异常或无效响应时，自动刷新游客会话并重试一次；失败后仍回退分享页解析。
- 抖音：视频默认优先最高画质，并为普通视频、图片和动图保留多个候选地址；地址不可用或超过大小限制时会继续尝试后续候选。
- 小红书：补全 Live Photo 视频候选地址、视频下载回退，并修正图片、`file_id` 与 Live Photo 的索引对齐。
- 新增 `gmssl` 依赖，用于抖音游客请求签名；补充相应第三方授权说明与回归测试。

## v1.7.9

- 更新avif压制参数

## v1.7.8

- 限制 FFmpeg 元数据参数长度，避免完整 EXIF/XMP JSON 进入命令行并触发 WinError 206；完整元数据仍保存在旁路 JSON，FFmpeg 保留基础映射。
- 预览 JPG 将能正常生成，因此不再因降级发送巨大的升图 PNG 而增加合并转发超时风险。
- 添加超大元数据回归测试，验证 FFmpeg 参数总长度受控且元数据 JSON 保持有效。

## v1.7.7

- 修复升图和ffmpeg并发开关不生效

## v1.7.6

- allow_ai_upscale_ffmpeg_concurrent（开关，默认关闭）：控制 AI 升图和 FFmpeg 能否同时工作。关闭时两者互斥、不会同时运行；开启后可并行。ai_upscale_max_concurrent（1-8，默认 1）：AI 升图最多同时运行几个任务。ffmpeg_max_concurrent（1-8，默认 1）：FFmpeg 转码最多同时运行几个任务。

## v1.7.5

- v1.7.5 自动更新
- 新增全局并发设置：AI 升图与 FFmpeg 可分别设置并行任务数，并可选择两者是否同时运行。

## v1.7.4

- 通用设置已分为：AI 升图、解析与交互、字体与卡片渲染、媒体发送、媒体处理与运行。

## v1.7.3

- Upscayl 双重生图 已迁至 general_settings.upscayl_double_pass，平台自动图片处理统一使用该设置。

## v1.7.2

- v1.7.2 自动更新

## v1.7.1

- 新增 12 个扩展平台的通用下载通道：快手、视频号、知乎、小黑盒、A站、YouTube、TikTok、Instagram、Pixiv、Iwara、网易云、NGA

## v1.6.4

- v1.6.4 自动更新

## v1.6.3

- v1.6.3 自动更新

## v1.6.2

- v1.6.2 自动更新

## v1.6.1

- v1.6.1 自动更新

## v1.6.0

- 图片支持保留元数据及额外数据.

## v1.5.5

- v1.5.5 自动更新

## v1.5.4

- v1.5.4 自动更新

## v1.5.3

- 修复bug

## v1.5.2

- 已增强同消息图片触发：
- 支持图片组件附带 /升图
- 支持字典格式图片消息
- 支持 CQ 图片消息中的 URL
- 支持 url、file、path 字段
- 支持引用图片和本地文件路径
- 支持 /升图 标准 与图片同时发送
- 已扩展图片处理标注。现在会显示：
- 原始: 107.59 KB -> AI升图后: 1.24 MB (1152.0%) -> 转码后: 872.87 KB (811.3%)
- 四个平台的 AI 升图中间文件都会传入标注逻辑，包括小红书、抖音、微博和 X。

## v1.5.1

- /生图进度 改为 /升图进度
- /avif进度 保留为兼容入口，共用同一套状态逻辑
- 移除了图片工具重复注册的进度处理
- 全插件同时只允许一个 AI 升图或 FFmpeg 任务执行
- 平台自动处理和手动 /升图、/avif 共用锁
- 修正小红书处理链未经过共享锁的问题已加入模型选择：
- /升图：使用配置中的模型
- /升图 自动：启用 CV 检测并自动选择模型
- /升图 二次元
- /升图 高保真
- /升图 Remacri
- /升图 超混合平衡
- /升图 超锐化
- /升图 轻量
- /升图 标准
- 配置中新增了独立图片工具的模型选择项，默认支持 CV 自动判定。


## v1.5.0

- 公共逻辑已集中到 [annotations.py](D:\\飞牛双向同步\\AstrBot插件\\小红书等多平台无水印下载\\astrbot_plugin_link_resolver_plus\\core\\common\\media\\annotations.py)，统一负责模型显示名、原始/转码后体积、百分比和标注文案；各平台 handler 不再各自维护格式。升图公共方法也会返回分类与模型元数据，避免各平台标注不一致。

## v1.4.7

- 抖音只要下载到图片，就执行 AVIF 转码和独立文件发送，不再受 video_url 元数据影响。

## v1.4.6

- 抖音：此前只发合并转发里的 JPG 预览，未独立发送 AVIF。
- 微博、X：正常生成 JPG 预览时也会漏发 AVIF，只在预览失败时才尝试上传。
- 小红书：原本已有合并消息后独立发送 AVIF 的逻辑，无需修改。
- B站没有走图片 AVIF 后处理链路。

## v1.4.5

- 已将 ffmpeg_bin_path 从 xhs_settings 移至 general_settings，配置读取也已改为 general_settings.ffmpeg_bin_path，并更新说明为全平台用途。
- 自动将 //... 补全为 https://...
- 依次尝试 masterUrl、url、backupUrl 和 backupUrls
- 跳过无效视频流，尝试同编码的后续流和其他编码

## v1.4.4

- 移除小红书专属 AVIF 压缩配置及其加载逻辑。
- 小红书图片后处理改为读取全局 general_settings.enable_ffmpeg_compress。
- 全局开关说明更新为覆盖小红书、抖音、微博和 X。
- 同步修正 MOD_DESCRIPTION.md 中的旧配置路径说明。

## v1.4.3

- 配置新增 private_filter.mode 和 private_filter.user_list，默认黑名单空列表，保持私聊全部放行。
- 所有链接解析入口和 JSON 卡片入口都会按私聊发送者 QQ 号过滤。
- 群聊仍使用原有群号过滤逻辑，互不影响。
- 增加私聊过滤测试用例。

## Unreleased

- 新增私聊黑名单/白名单配置，可按私聊用户 QQ 号控制链接解析。

## v1.4.2

- 合并阿普升图可执行文件及模型文件路径到通用设置中

## v1.4.1

- 更改插件名

## v1.4.0

- 现在以下视频都会在 AstrBot 下载完成后，复制到：
- \\192.168.1.34\Docker\NapCat\config\astrbot-media
- 随后让 NapCat 容器从：
- /app/napcat/config/astrbot-media
- 读取并发送：
- Bilibili，包括多 P 视频
- 抖音普通视频、动图
- 小红书视频、Live Photo
- 微博视频
- X 视频

## v1.3.0

- 修复AI升图处理标注中的占位符问题，现在图片类型会明确显示为“二次元(CV)”、“照片(CV)”或“手动指定”，实际升图模型名称如“标准(upscayl-standard-4x)”、“数字艺术(digital-art-4x)”、“超锐化(ultrasharp-4x)”等会准确展示，不再出现unknown或auto回退值；改动涉及小红书handler及标注生成逻辑，重启AstrBot后生效。

## v1.2.7

- 修复AstrBot与NapCat不在同一主机时视频发送ENOENT错误，改为将本地缓存视频通过文件服务注册为HTTP URL后发送，覆盖B站、抖音、小红书、微博、X/Twitter等平台；需确保AstrBot文件服务地址对NapCat可达（非127.0.0.1），并在配置中设置正确主机地址后重启生效；本地编译与差异检查通过。

## v1.2.6

- 添加logo

## v1.2.5

- v1.2.5 自动更新

## v1.2.4

- v1.2.4 自动更新

## v1.2.3

- v1.2.3 自动更新

## v1.2.2

- v1.2.2 自动更新

## v1.2.1

- 修复检测分辨率bug

## v1.2

- 加入自动选择模型选择

## v1.1

- 去掉ONNX模型识别功能,全程使用CV模型

## v1.0.30

- 取消内置阿普升图,用户自行安装

## v1.0.20

- v1.0.20 自动更新

## v1.0.10

### 新增

- 新增微博图文和视频解析，支持原图优先、访客 Cookie 和自定义 Cookie
- 新增 X/Twitter 图文和视频解析，支持 `twitter.com` 与 `x.com`
- 新增群黑名单/白名单配置，私聊始终放行
- 新增多表情回应，可选择随机一个或按顺序依次回应；显式空列表表示不回应
- B站、抖音和小红书新增文字摘要模式，可按需切换回渲染卡片
- 新增自定义字体与托管字体安装，字体优先级为：用户配置 > 插件托管字体 > 系统字体

### 优化

- 扩展中文字体查找范围，并增加 `fc-match` 兜底
- 托管字体下载移到后台线程，避免阻塞 AstrBot 事件循环
- 插件配置、Cookie、字体和缓存统一保存在 AstrBot 插件数据目录
- 微博正文优先展开长文，图片优先原图/大图，视频优先最高码率
- JSON 卡片入口与文本链接入口统一应用平台开关和群过滤

### 修复

- 修复 B站部分画质枚举值为空时的崩溃
- 兼容 B站 `hvc1` / `hev1` HEVC 编码标签
- 修复跨机器部署时合并转发视频使用本地 `file://` 路径导致 NapCat `ENOENT`
- 修复微博和 X 发送失败时未清理临时媒体文件
- 修复小红书入口层把协程当作异步生成器迭代，导致 `async for requires __aiter__` 的问题
- 修复小红书卡片渲染配置刷新后未及时生效

## v1.0.9

### 变更

- 配置面板改为分组折叠结构，配置项迁移为按平台分组
- 小红书移除 Cookies 支持，优化图片下载逻辑（CDN 原图 / CI-PNG 转码）
- 新增「小红书图片并发下载」配置项
- 新增「小红书优先 CI-PNG 转码」配置项
- 新增「小红书图片转文件阈值(MB)」配置项，超限图片自动转为文件发送
- 调整抖音 / 小红书视频合并发送默认值为关闭

### 修复

- 修复小红书视频下载链路参数缺失导致的异常
- 修复小红书解析超大图时 pipeline 错误

### ⚠️ 升级须知

- v1.0.8 及更早版本升级时，配置项会按新结构重建，建议事先复制 B 站 Cookies，升级后重新粘贴

## v1.0.8

### 修复

- 修复 v1.0.6 修改 UA 导致抖音解析失败的问题

## v1.0.7

### 新增

- 新增 `error_notify_mode` 配置项，控制解析/下载失败时 Bot 对用户的反馈行为：
  - **静默**（默认）：仅记录日志，不向群内发送任何错误消息
  - **脱敏**：发送通用错误提示（如"视频下载失败"），不包含异常详情
  - **报错**：发送完整错误信息，包含异常详情（旧版默认行为）

### 修复

- 修复 B 站视频下载/发送失败时将 `str(exc)` 直接发送给用户，可能泄露内部信息（文件路径、网络错误详情、API 响应等）的问题

## v1.0.6

### 新增

- 新增 `merge_send_as_sender` 配置项，合并转发的发送人可显示为发送链接的用户

### 优化

- ~~引入 `CHROME_UA` 常量，统一各平台 User-Agent~~（已在 v1.0.8 回退）
- 解析任务管理从全局变量迁移为实例级任务池，插件重载时自动清理旧任务
- 统一使用 `time.perf_counter()` 记录耗时
- 优化小红书图片下载逻辑
- 优化日志记录格式

### 移除

- 移除 API 超时时间配置项（无实际作用）
- 移除自动清理延迟配置项（无实际作用）

## v1.0.5

### 修复

- 修复 B 站并发发送时 ENOENT 竞态：下载文件增加 `request_id` 后缀避免同名冲突，发送前增加文件存在性校验

## v1.0.4

### 新增

- 新增 `bili_max_duration_seconds` 配置项，支持 B 站视频最大时长限制
- 增强小红书重试机制与错误处理

## v1.0.3

### 修复

- 修复短链接解析异常处理并优化图片下载逻辑

## v1.0.2

### 优化

- 重构路径管理，改善 Cookie 处理逻辑

## v1.0.1

### 新增

- 初始版本：支持 B 站 / 抖音 / 小红书链接自动解析
- 插件配置页面支持粘贴 B 站 cookies.txt
